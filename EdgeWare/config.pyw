...
INFO_DESCRIPTION_DEFAULT = (
    "No pack loaded, or the pack does not have an 'info.json' file."
)
INFO_CREATOR_DEFAULT = 'Anonymous'
INFO_VERSION_DEFAULT = '0'
INFO_DISCORD_DEFAULT = [
    "[No pack loaded, or the pack does not have a 'discord.dat' file.]",
    'default',
]
...
if info_id == '0' and os.path.exists(Resource.ROOT):
    try:
        ...
        im = (
            str(len(os.listdir(Resource.IMAGE)))
            if os.path.exists(Resource.IMAGE)
            else '0'
        )
        au = (
            str(len(os.listdir(Resource.AUDIO)))
            if os.path.exists(Resource.AUDIO)
            else '0'
        )
        vi = (
            str(len(os.listdir(Resource.VIDEO)))
            if os.path.exists(Resource.VIDEO)
            else '0'
        )
        ...
        errors_list.append(
            'Could not create the pack unique ID! Mood settings might not save!\n'
        )
...
if settings['toggleMoodSet'] != True:
    ...
    if MOOD_PATH != '0' and not os.path.isfile(MOOD_PATH):
        logging.info(
            f'moods file does not exist, creating one using unique id {UNIQUE_ID}'
        )
        try:
...
def show_window():
    ...
    while fail_loop < 2:
        try:
            ...
            hibernateTypeVar = StringVar(
                root, value=(settings['hibernateType'].strip())
            )
            ...
            subliminalsChanceVar = IntVar(
                root, value=int(settings['subliminalsChance'])
            )
            maxSubliminalsVar = IntVar(root, value=int(settings['maxSubliminals']))

            safeModeVar = BooleanVar(root, value=(int(settings['safeMode']) == 1))

            antiOrLanczosVar = BooleanVar(
                root, value=(int(settings['antiOrLanczos']) == 1)
            )
            toggleInternetVar = BooleanVar(
                root, value=(int(settings['toggleInternet']) == 1)
            )
            toggleHibSkipVar = BooleanVar(
                root, value=(int(settings['toggleHibSkip']) == 1)
            )
            toggleMoodSetVar = BooleanVar(
                root, value=(int(settings['toggleMoodSet']) == 1)
            )

            buttonlessVar = BooleanVar(root, value=(int(settings['buttonless']) == 1))
            captionFilenameVar = BooleanVar(
                root, value=(int(settings['captionFilename']) == 1)
            )
            singleModeVar = BooleanVar(root, value=(int(settings['singleMode']) == 1))

            corruptionModeVar = BooleanVar(
                root, value=(int(settings['corruptionMode']) == 1)
            )
            corruptionDevVar = BooleanVar(
                root, value=(int(settings['corruptionDevMode']) == 1)
            )
            corruptionFullPermVar = BooleanVar(
                root, value=(int(settings['corruptionFullPerm']) == 1)
            )
            ...
            corruptionLaunchesVar = IntVar(
                root, value=int(settings['corruptionLaunches'])
            )
            corruptionFadeTypeVar = StringVar(
                root, value=(settings['corruptionFadeType'].strip())
            )
            corruptionTriggerVar = StringVar(
                root, value=(settings['corruptionTrigger'].strip())
            )
            corruptionWallpaperCycleVar = BooleanVar(
                root, value=(int(settings['corruptionWallpaperCycle']) == 1)
            )
            corruptionThemeCycleVar = BooleanVar(
                root, value=(int(settings['corruptionThemeCycle']) == 1)
            )
            corruptionPurityModeVar = BooleanVar(
                root, value=(int(settings['corruptionPurityMode']) == 1)
            )
            ...
            themeNoConfigVar = BooleanVar(
                root, value=(int(settings['themeNoConfig']) == 1)
            )

            presetsDangerVar = BooleanVar(
                root, value=(int(settings['presetsDanger']) == 1)
            )
            ...
            sMessageTransparencyVar = IntVar(
                root, value=int(settings['sMessageTransparency'])
            )

            movingChanceVar = IntVar(root, value=int(settings['movingChance']))
            movingSpeedVar = IntVar(root, value=int(settings['movingSpeed']))
            movingRandomVar = BooleanVar(
                root, value=(int(settings['movingRandom']) == 1)
            )
    ...
    hibernateTypeDescriptionFrame = Frame(
        hibernateHostFrame, borderwidth=2, relief=GROOVE
    )
    ...
    toggleHibernateButton = Checkbutton(
        ...
        command=lambda: hibernateHelper(hibernateTypeVar.get()),
        cursor='question_arrow',
    )
    fixWallpaperButton = Checkbutton(
        hibernateTypeFrame,
        text='Fix Wallpaper',
        variable=fixWallpaperVar,
        cursor='question_arrow',
    )
    hibernateTypeDropdown = OptionMenu(
        hibernateTypeFrame,
        hibernateTypeVar,
        *hibernate_types,
        command=lambda key: hibernateHelper(key),
    )
    hibernateTypeDescription = Label(
        hibernateTypeDescriptionFrame,
        text='Error loading Hibernate Description!',
        wraplength=175,
    )
    ...
    def hibernateHelper(key: str):
        if key == 'Original':
            hibernateTypeDescription.configure(
                text='Creates an immediate quantity of popups on wakeup based on the awaken activity.\n\n'
            )
            if hibernateVar.get():
        ...
        if key == 'Spaced':
            hibernateTypeDescription.configure(
                text='Creates popups consistently over the hibernate length, based on popup delay.\n\n'
            )
            if hibernateVar.get():
        ...
        if key == 'Chaos':
            hibernateTypeDescription.configure(
                text='Every time hibernate activates, a random type (other than chaos) is selected.\n\n'
            )
            if hibernateVar.get():
                toggleAssociateSettings(True, hlength_group)
                toggleAssociateSettings(True, hactivity_group)
    ...
    h_activityButton = Button(
        ...
        command=lambda: assign(
            wakeupActivityVar,
            simpledialog.askinteger('Manual Wakeup Activity', prompt='[1-50]: '),
        ),
    )
    hibernateLengthScale = Scale(
        hibernateLengthFrame,
        label='Max Length (sec)',
        variable=hibernateLengthVar,
        orient='horizontal',
        from_=5,
        to=300,
    )
    hibernateLengthButton = Button(
        ...
        command=lambda: assign(
            hibernateLengthVar,
            simpledialog.askinteger('Manual Hibernate Length', prompt='[5-300]: '),
        ),
    )
    ...
    Label(tabGeneral, text='Hibernate Settings', font=titleFont, relief=GROOVE).pack(
        pady=2
    )
    ...
    hibernateTypeDescriptionFrame.pack(
        fill='both', side='left', expand=1, padx=2, pady=2
    )
    ...
    toggleFlairButton = Checkbutton(
        ...
        variable=startFlairVar,
        cursor='question_arrow',
    )
    toggleROSButton = Checkbutton(
        toggleFrame2, text='Run Edgeware on Save & Exit', variable=rosVar
    )
    toggleDesktopButton = Checkbutton(
        toggleFrame3, text='Create Desktop Icons', variable=deskIconVar
    )
    toggleSafeMode = Checkbutton(
        toggleFrame3,
        text='Warn if "Dangerous" Settings Active',
        variable=safeModeVar,
        cursor='question_arrow',
    )
    ...
    discordttp = CreateToolTip(
        toggleDiscordButton,
        'Displays a lewd status on discord (if your discord is open), which can be set per-pack by the pack creator.',
    )
    ...
    web_verLabel = Label(
        verFrame,
        text=f'EdgeWare GitHub Version:\n{webv}',
        bg=(BUTTON_FACE if (defaultVars[0] == webv) else 'red'),
    )
    openGitButton = Button(
        zipGitFrame,
        text='Open Github (EdgeWare Base)',
        command=lambda: webbrowser.open('https://github.com/PetitTournesol/Edgeware'),
    )

    verPlusFrame = Frame(infoHostFrame)
    local_verPlusLabel = Label(
        verPlusFrame, text=f'EdgeWare++ Local Version:\n{defaultVars[1]}'
    )
    web_verPlusLabel = Label(
        verPlusFrame,
        text=f'EdgeWare++ GitHub Version:\n{webvpp}',
        bg=(BUTTON_FACE if (defaultVars[1] == webvpp) else 'red'),
    )
    openGitPlusButton = Button(
        zipGitFrame,
        text='Open Github (EdgeWare++)',
        command=lambda: webbrowser.open('https://github.com/araten10/EdgewarePlusPlus'),
    )
    ...
    exportResourcesButton = Button(
        resourceFrame, text='Export Resource Pack', command=exportResource
    )
    importResourcesButton = Button(
        resourceFrame, text='Import Resource Pack', command=lambda: importResource(root)
    )
    ...
    themeDropdown = OptionMenu(
        subThemeFrame, themeTypeVar, *theme_types, command=lambda key: themeHelper(key)
    )
    themeDropdown.configure(width=12)
    ignoreConfigToggle = Checkbutton(
        subThemeFrame,
        text='Ignore Config',
        variable=themeNoConfigVar,
        cursor='question_arrow',
    )

    ignoreconfigttp = CreateToolTip(
        ignoreConfigToggle,
    ...
    def themeHelper(theme):
        ...
        if theme == 'Dark':
            for widget in all_children(testThemeFrame):
                ...
                if isinstance(widget, Label):
                    widget.configure(
                        bg='#282c34', fg='ghost white', font=('Segoe UI', 9)
                    )
                if isinstance(widget, OptionMenu):
                    widget.configure(
                        bg='#282c34',
        ...
        if theme == 'Goth':
            for widget in all_children(testThemeFrame):
                ...
                if isinstance(widget, Label):
                    widget.configure(
                        bg='#282c34', fg='MediumPurple1', font=('Constantia', 9)
                    )
                if isinstance(widget, OptionMenu):
                    widget.configure(
                        bg='#282c34',
    ...
    testPopupLabel = Label(
        testThemePopup,
        image=testPopupImage,
        width=150,
        height=75,
        borderwidth=2,
        relief=GROOVE,
    ...
    testPromptBody = Frame(
        testThemePrompt, borderwidth=2, relief=GROOVE, width=150, height=75
    )
    ...
    testScaleActivated = Scale(
        testCColumn2, orient='horizontal', from_=1, to=100, highlightthickness=0
    )
    testButtonActivated = Button(testCColumn2, text='Activated')

    testCColumn3 = Frame(testConfigBody)
    testScaleDeactivated = Scale(
        testCColumn3, orient='horizontal', from_=1, to=100, highlightthickness=0
    )
    ...
    testPopupButton.place(
        x=140 - testPopupButton.winfo_reqwidth(),
        y=70 - testPopupButton.winfo_reqheight(),
    )
    ...
    mitosisttp = CreateToolTip(
        mitosisToggle,
        "When a popup is closed, more popups will spawn in it's place based on the mitosis strength.",
    )
    ...
    doPanicButton = Button(
        panicFrame,
        text='Perform Panic',
        command=lambda: subprocess.Popen([sys.executable, Process.PANIC]),
    )

    setpanicttp = CreateToolTip(
        setPanicButtonButton,
        'NOTE: To use this hotkey you must be "focused" on a EdgeWare popup. Click on a popup before using.',
    )
    ...
    movingSlider = Scale(
        ...
        label='Moving Chance',
        orient='horizontal',
        variable=movingChanceVar,
        cursor='question_arrow',
    )
    movingRandToggle = Checkbutton(
        movingFrame,
        text='Rand. Direction',
        variable=movingRandomVar,
        cursor='question_arrow',
    )
    ...
    moverandomttp = CreateToolTip(
        movingRandToggle,
        'Makes moving popups move in a random direction rather than the static diagonal one.',
    )

    movingSpeedSlider = Scale(
        speedFrame,
        label='Max Movespeed',
        from_=1,
        to=15,
        orient='horizontal',
        variable=movingSpeedVar,
    )
    manualSpeed = Button(
        speedFrame,
        text='Manual speed...',
        command=lambda: assign(
            movingSpeedVar, simpledialog.askinteger('Manual Speed', prompt='[1-15]: ')
        ),
    )

    denialttp = CreateToolTip(
        denialToggle, 'Adds a percentage chance to "censor" an image.'
    )
    ...
    popupWebToggle = Checkbutton(
        popupOptionsSubFrame1, text='Popup close opens web page', variable=popupWebVar
    )
    toggleEasierButton = Checkbutton(
        popupOptionsSubFrame2,
        text='Buttonless Closing Popups',
        variable=buttonlessVar,
        cursor='question_arrow',
    )
    toggleSingleButton = Checkbutton(
        popupOptionsSubFrame3,
        text='Single Popup Mode',
        variable=singleModeVar,
        cursor='question_arrow',
    )
    toggleMultiClickButton = Checkbutton(
        popupOptionsSubFrame3,
        text='Multi-Click popups',
        variable=multiClickVar,
        cursor='question_arrow',
    )
    ...
    maxAudioToggle = Checkbutton(
        maxAudioFrame,
        text='Cap Audio',
        variable=maxAToggleVar,
        command=lambda: toggleAssociateSettings(maxAToggleVar.get(), maxAudio_group),
    )
    maxAudioScale = Scale(
        maxAudioFrame,
        label='Max Audio Popups',
        from_=1,
        to=50,
        orient='horizontal',
        variable=maxAudioVar,
    )
    maxAudioManual = Button(
        maxAudioFrame,
        text='Manual Max Audio...',
        command=lambda: assign(
            maxAudioVar, simpledialog.askinteger('Manual Max Audio', prompt='[1-50]: ')
        ),
    )
    ...
    maxVideoToggle = Checkbutton(
        maxVideoFrame,
        text='Cap Videos',
        variable=maxVToggleVar,
        command=lambda: toggleAssociateSettings(maxVToggleVar.get(), maxVideo_group),
    )
    maxVideoScale = Scale(
        maxVideoFrame,
        label='Max Video Popups',
        from_=1,
        to=50,
        orient='horizontal',
        variable=maxVideoVar,
    )
    maxVideoManual = Button(
        maxVideoFrame,
        text='Manual Max Videos...',
        command=lambda: assign(
            maxVideoVar, simpledialog.askinteger('Manual Max Videos', prompt='[1-50]: ')
        ),
    )
    ...
    subliminalsChanceManual = Button(
        ...
        command=lambda: assign(
            subliminalsChanceVar,
            simpledialog.askinteger('Manual Subliminal Chance', prompt='[1-100]: '),
        ),
    )
    ...
    maxSubliminalsScale = Scale(
        maxSubliminalsFrame,
        label='Max Subliminals',
        from_=1,
        to=200,
        orient='horizontal',
        variable=maxSubliminalsVar,
    )
    maxSubliminalsManual = Button(
        ...
        command=lambda: assign(
            maxSubliminalsVar,
            simpledialog.askinteger('Manual Max Subliminals', prompt='[1-200]: '),
        ),
    )
    ...
    toggleFilenameButton = Checkbutton(
        captionsSubFrame1,
        text='Use filename for caption moods',
        variable=captionFilenameVar,
        cursor='question_arrow',
    )
    toggleMultiClickButton = Checkbutton(
        captionsSubFrame1,
    ...
    toggleCaptionMood = Checkbutton(
        captionsSubFrame1,
        text='Use Cap-Pop specific mood',
        variable=capPopMoodVar,
        cursor='question_arrow',
    )

    captionttp = CreateToolTip(
        toggleCaptionsButton,
        'Enables captions on popups. These are short segments of text written by the pack creator that adorn the top of each popup.',
    )
    ...
    captionsPopupSlider = Scale(
        capPopFrame,
        label='Cap-Pop Chance',
        from_=0,
        to=100,
        orient='horizontal',
        variable=capPopChanceVar,
    )
    captionsPopupManual = Button(
        ...
        command=lambda: assign(
            capPopChanceVar,
            simpledialog.askinteger(
                'Manual Caption Popup Chance (%)', prompt='[0-100]: '
            ),
        ),
        cursor='question_arrow',
    )
    capPopOpacitySlider = Scale(
        capPopOpacityFrame,
        label='Cap-Pop Opacity',
        from_=1,
        to=100,
        orient='horizontal',
        variable=capPopOpacityVar,
    )
    capPopOpacityManual = Button(
        ...
        command=lambda: assign(
            capPopOpacityVar,
            simpledialog.askinteger(
                'Manual Caption Popup Opacity (%)', prompt='[1-100]: '
            ),
        ),
    )
    ...
    capPopTimerManual = Button(
        ...
        command=lambda: assign(
            capPopTimerVar,
            simpledialog.askinteger(
                'Manual Caption Popup Timer (ms)', prompt='[1-1000]: '
            ),
        ),
    )
    ...
    corruptionToggle = Checkbutton(
        corruptionStartFrame,
        text='Turn on Corruption',
        variable=corruptionModeVar,
        cursor='question_arrow',
    )
    corruptionFullToggle = Checkbutton(
        corruptionStartFrame,
        text='Full Permissions Mode',
        variable=corruptionFullPermVar,
        cursor='question_arrow',
    )
    corruptionRecommendedToggle = Button(
        ...
        cursor='question_arrow',
        height=2,
        command=lambda: packPreset(
            in_var_group, in_var_names, 'corruption', presetsDangerVar.get()
        ),
    )
    ...
    fadeDropdown = OptionMenu(
        fadeSubInfo,
        corruptionFadeTypeVar,
        *fade_types,
        command=lambda key: fadeHelper(key),
    )
    fadeDropdown.configure(width=9, highlightthickness=0)
    fadeDescription = Label(
        fadeInfoFrame,
        text='Error loading fade description!',
        borderwidth=2,
        relief=GROOVE,
        wraplength=150,
    )
    ...
    fadeImageContainer = Label(
        fadeSubInfo, image=fadeImageNormal, borderwidth=2, relief=GROOVE
    )
    trigger_types = ['Timed', 'Popup', 'Launch']
    triggerDropdown = OptionMenu(
        triggerSubInfo,
        corruptionTriggerVar,
        *trigger_types,
        command=lambda key: triggerHelper(key, False),
    )
    triggerDropdown.configure(width=9, highlightthickness=0)
    triggerDescription = Label(
        triggerInfoFrame,
        text='Error loading trigger description!',
        borderwidth=2,
        relief=GROOVE,
        wraplength=150,
    )
    ...
    corruptionTimerButton = Button(
        ...
        command=lambda: assign(
            corruptionTimeVar,
            simpledialog.askinteger('Manual Level Time (sec)', prompt='[5-1800]: '),
        ),
    )
    corruptionTimerScale = Scale(
        cTimerFrame,
        label='Level Time',
        variable=corruptionTimeVar,
        orient='horizontal',
        from_=5,
        to=1800,
    )
    ...
    corruptionPopupsButton = Button(
        ...
        command=lambda: assign(
            corruptionPopupsVar,
            simpledialog.askinteger(
                'Manual Level Popups (per transition)', prompt='[3-100]: '
            ),
        ),
    )
    corruptionPopupsScale = Scale(
        cPopupsFrame,
        label='Level Popups',
        variable=corruptionPopupsVar,
        orient='horizontal',
        from_=3,
        to=100,
    )
    ...
    corruptionLaunchesButton = Button(
        ...
        command=lambda: assign(
            corruptionLaunchesVar,
            simpledialog.askinteger(
                'Manual Level Launches (per transition)', prompt='[2-31]: '
            ),
        ),
    )
    corruptionLaunchesScale = Scale(
        cLaunchesFrame,
        label='Level Launches',
        variable=corruptionLaunchesVar,
        orient='horizontal',
        from_=2,
        to=31,
    )
    cOtherTimerFrame = Frame(corruptionTimeFrame)
    clearLaunchesButton = Button(
        cOtherTimerFrame,
        text='Reset Launches',
        height=3,
        command=lambda: clearLaunches(True),
    )
    ...
    def fadeHelper(key):
        if key == 'Normal':
            fadeDescription.configure(
                text='Gradually transitions between corruption levels.'
            )
            fadeImageContainer.configure(image=fadeImageNormal)
        if key == 'Abrupt':
            fadeDescription.configure(
                text='Immediately switches to new level upon timer completion.'
            )
            fadeImageContainer.configure(image=fadeImageAbrupt)
        if key == 'Noise':
            fadeDescription.configure(
                text='Scatters levels randomly across the time range.'
            )
            fadeImageContainer.configure(image=fadeImageNoise)

    def triggerHelper(key, tutorialMode):
        if key == 'Timed':
            triggerDescription.configure(
                text='Transitions based on time elapsed in current session.'
            )
            if tutorialMode:
                toggleAssociateSettings_manual(
                    True, ctime_group, 'lime green', 'forest green'
                )
                toggleAssociateSettings_manual(
                    False, cpopup_group, 'lime green', 'forest green'
                )
                toggleAssociateSettings_manual(
                    False, claunch_group, 'lime green', 'forest green'
                )
            else:
        ...
        if key == 'Popup':
            triggerDescription.configure(
                text='Transitions based on number of popups in current session.'
            )
            if tutorialMode:
                toggleAssociateSettings_manual(
                    False, ctime_group, 'lime green', 'forest green'
                )
                toggleAssociateSettings_manual(
                    True, cpopup_group, 'lime green', 'forest green'
                )
                toggleAssociateSettings_manual(
                    False, claunch_group, 'lime green', 'forest green'
                )
        ...
        if key == 'Launch':
            triggerDescription.configure(
                text='Transitions based on number of EdgeWare launches.'
            )
            if tutorialMode:
                toggleAssociateSettings_manual(
                    False, ctime_group, 'lime green', 'forest green'
                ...
                toggleAssociateSettings_manual(
                    True, claunch_group, 'lime green', 'forest green'
                )
            else:
                toggleAssociateSettings(False, ctime_group)
                toggleAssociateSettings(False, cpopup_group)
    ...
    corruptionTransitionBody = Label(
        cTabTransitions, text=CTRANSITION_TEXT, wraplength=300
    )
    ...
    corruptionWallpaperToggle = Checkbutton(
        corruptionAddSub1,
        text="Don't Cycle Wallpaper",
        variable=corruptionWallpaperCycleVar,
        cursor='question_arrow',
    )
    corruptionThemeToggle = Checkbutton(
        corruptionAddSub1,
        text="Don't Cycle Themes",
        variable=corruptionThemeCycleVar,
        cursor='question_arrow',
    )
    corruptionPurityToggle = Checkbutton(
        corruptionAddSub2,
        text='Purity Mode',
        variable=corruptionPurityModeVar,
        cursor='question_arrow',
    )
    corruptionDevToggle = Checkbutton(
        corruptionAddSub2,
        text='Corruption Dev View',
        variable=corruptionDevVar,
        cursor='question_arrow',
    )
    ...
    corrwallpaperttp = CreateToolTip(
        ...
        'Prevents the wallpaper from cycling as you go through corruption levels, instead staying at '
    ...
    pathTree = ttk.Treeview(
        pathInnerFrame, height=6, show='headings', columns=('level', 'moods')
    )
    pathScrollbarY = ttk.Scrollbar(
        corruptionPathFrame, orient='vertical', command=pathTree.yview
    )
    pathScrollbarX = ttk.Scrollbar(
        pathInnerFrame, orient='horizontal', command=pathTree.xview
    )
    ...
    pHoldImageR = Image.open(Defaults.PANIC_WALLPAPER).resize(
    ...
    panicWallpaperButton = Button(
        panicWPFrameL,
        text='Change Panic Wallpaper',
        command=updatePanicPaper,
        cursor='question_arrow',
    )
    ...
    statusPackFrameVarLabel = Label(
        statusPackFrame,
        text=('✓' if statusPack else '✗'),
        font='Default 14',
        fg=('green' if statusPack else 'red'),
    )
    statusAboutFrameVarLabel = Label(
        statusAboutFrame,
        text=('✓' if statusAbout else '✗'),
        font='Default 14',
        fg=('green' if statusAbout else 'red'),
    )
    statusWallpaperFrameVarLabel = Label(
        statusWallpaperFrame,
        text=('✓' if statusWallpaper else '✗'),
        font='Default 14',
        fg=('green' if statusWallpaper else 'red'),
    )
    statusStartupFrameVarLabel = Label(
        statusStartupFrame,
        text=('✓' if statusStartup else '✗'),
        font='Default 14',
        fg=('green' if statusStartup else 'red'),
        cursor='question_arrow',
    )
    statusDiscordFrameVarLabel = Label(
        statusDiscordFrame,
        text=('✓' if statusDiscord else '✗'),
        font='Default 14',
        fg=('green' if statusDiscord else 'red'),
    )
    statusIconFrameVarLabel = Label(
        statusIconFrame,
        text=('✓' if statusIcon else '✗'),
        font='Default 14',
        fg=('green' if statusIcon else 'red'),
        cursor='question_arrow',
    )
    ...
    if os.path.exists(Resource.WEB):
        try:
            ...
            errors_list.append(
                'Something is wrong with the currently loaded web.json file!\n'
            )
            webStat = 0
    else:
        webStat = 0
    ...
    if os.path.exists(Resource.PROMPT):
        ...
        try:
            ...
            errors_list.append(
                'Something is wrong with the currently loaded prompt.json file!\n'
            )
            promptStat = 0
    else:
        promptStat = 0
    ...
    if os.path.exists(Resource.CAPTIONS):
        try:
            ...
            errors_list.append(
                'Something is wrong with the currently loaded captions.json file!\n'
            )
            captionStat = 0
    else:
        captionStat = 0
    ...
    subliminalStat = (
        len(os.listdir(Resource.SUBLIMINALS))
    ...
    ttk.Separator(imageStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=10
    )
    Label(imageStatsFrame, text=f'{imageStat}').pack(pady=2, side='top')
    audioStatsFrame.pack(fill='x', side='left', expand=1)
    Label(audioStatsFrame, text='Audio Files', font='Default 10').pack(
        pady=2, side='top'
    )
    ttk.Separator(audioStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=10
    )
    ...
    ttk.Separator(videoStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=10
    )
    ...
    ttk.Separator(webStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=10
    )
    ...
    ttk.Separator(promptStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=20
    )
    Label(promptStatsFrame, text=f'{promptStat}').pack(pady=2, side='top')
    captionsStatsFrame.pack(fill='x', side='left', expand=1)
    Label(captionsStatsFrame, text='Captions', font='Default 10').pack(
        pady=2, side='top'
    )
    ttk.Separator(captionsStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=20
    )
    Label(captionsStatsFrame, text=f'{captionStat}').pack(pady=2, side='top')
    subliminalsStatsFrame.pack(fill='x', side='left', expand=1)
    Label(subliminalsStatsFrame, text='Subliminals', font='Default 10').pack(
        pady=2, side='top'
    )
    ttk.Separator(subliminalsStatsFrame, orient='horizontal').pack(
        fill='x', side='top', padx=20
    )
    ...
    descriptionVarLabel = Label(
        descriptionFrame, text=infoDescriptionWrap.fill(text=f'{info_description}')
    )
    ...
    discordStatusLabel = Label(
        discordStatusFrame, text='Custom Discord Status:', font='Default 10'
    )
    discordStatusImageLabel = Label(
        discordStatusFrame, text='Discord Status Image:', font='Default 10'
    )
    if statusDiscord:
        try:
            ...
            errors_list.append(
                'Something is wrong with the currently loaded discord.dat file!\n'
            )
    ...
    discordStatusImageVarLabel = Label(
        discordStatusFrame, text=f'{info_discord[1]}', cursor='question_arrow'
    )
    ...
    configPresetsButton = Button(
        ...
        cursor='question_arrow',
        command=lambda: packPreset(
            in_var_group, in_var_names, 'full', presetsDangerVar.get()
        ),
    )
    ...
    if os.path.exists(Resource.CONFIG):
        ...
        configNum = 0
        toggleAssociateSettings(False, configpresets_group)
    configPresetsLabel = Label(
        configPresetsSub1, text=f'Number of suggested config settings: {configNum}'
    )
    presetsDangerToggle = Checkbutton(
        configPresetsSub1,
        text='Toggle on warning failsafes',
        variable=presetsDangerVar,
        cursor='question_arrow',
    )
    ...
    # Media frame
    mediaTree = CheckboxTreeview(
        moodsMediaFrame, height=7, show='tree', name='mediaTree'
    )
    mediaScrollbar = ttk.Scrollbar(
        moodsMediaFrame, orient=VERTICAL, command=mediaTree.yview
    )
        ...
        try:
            with open(Resource.MEDIA, 'r') as f:
                ...
                for m in l:
                    if m == 'default':
                        continue
                    parent = mediaTree.insert(
                        '', 'end', iid=str(m), values=str(m), text=str(m)
                    )
                    mediaTree.insert(
                        parent,
                        'end',
                        iid=(f'{m}desc'),
                        text=(f'{len(l[m])} media related to this mood.'),
                    )
            ...
            mediaTree.insert(
                ...
                'end',
                id='NAer',
                text="Pack doesn't support media moods, \nor they're improperly configured!",
            )
        ...
        if len(mediaTree.get_children()) != 0:
            if MOOD_PATH != '0' and os.path.exists(Resource.ROOT):
                try:
                    ...
                    errors_list.append(
                        "The media treeview nodes couldn't finish their checking setup!\n"
                    )
    ...
    # Captions frame
    captionsTree = CheckboxTreeview(
        moodsCaptionsFrame, height=7, show='tree', name='captionsTree'
    )
    captionsScrollbar = ttk.Scrollbar(
        moodsCaptionsFrame, orient=VERTICAL, command=captionsTree.yview
    )
    ...
    if os.path.exists(Resource.CAPTIONS):
        try:
            with open(Resource.CAPTIONS, 'r') as f:
                ...
                for m in l:
                    ...
                    captionsTree.insert(
                        ...
                        'end',
                        iid=(f'{m}desc'),
                        text=(f'{len(l[m])} captions related to this mood.'),
                    )
            ...
            captionsTree.insert(
                ...
                'end',
                iid='NAer',
                text="Pack doesn't support caption moods, \nor they're improperly configured!",
            )
        ...
        if len(captionsTree.get_children()) != 0:
            if MOOD_PATH != '0' and os.path.exists(Resource.ROOT):
                try:
                    ...
                    errors_list.append(
                        "The captions treeview nodes couldn't finish their checking setup!\n"
                    )
    ...
    # Prompts frame
    promptsTree = CheckboxTreeview(
        moodsPromptsFrame, height=7, show='tree', name='promptsTree'
    )
    promptsScrollbar = ttk.Scrollbar(
        moodsPromptsFrame, orient=VERTICAL, command=promptsTree.yview
    )
    ...
    if os.path.exists(Resource.PROMPT):
        try:
            with open(Resource.PROMPT, 'r') as f:
                ...
                for m in l['moods']:
                    ...
                    promptsTree.insert(
                        ...
                        'end',
                        iid=(f'{m}desc'),
                        text=(f'{len(l[m])} prompts related to this mood.'),
                    )
            ...
            promptsTree.insert(
                ...
                'end',
                iid='NAer',
                text="Pack doesn't support prompt moods, \nor they're improperly configured!",
            )
        ...
        if len(promptsTree.get_children()) != 0:
            if MOOD_PATH != '0' and os.path.exists(Resource.ROOT):
                try:
                    ...
                    errors_list.append(
                        "The prompt treeview nodes couldn't finish their checking setup!\n"
                    )
    ...
    if os.path.exists(Resource.WEB):
        try:
                ...
                for m in l['moods']:
                    ...
                    if m not in webMoodList:
                        ...
                        webTree.insert(
                            ...
                            'end',
                            iid=(f'{m}desc'),
                            text=(f'{mCount} web links related to this mood.'),
                        )
            ...
            webTree.insert(
                ...
                'end',
                iid='NAer',
                text="Pack doesn't support web moods, \nor they're improperly configured!",
            )
        ...
        if len(webTree.get_children()) != 0:
            if MOOD_PATH != '0' and os.path.exists(Resource.ROOT):
                try:
                    ...
                    errors_list.append(
                        "The web treeview nodes couldn't finish their checking setup!\n"
                    )
    ...
    fileTabImportButton = Button(
        importExportFrame,
        height=2,
        text='Import Resource Pack',
        command=lambda: importResource(root),
    )
    fileTabExportButton = Button(
        importExportFrame, height=2, text='Export Resource Pack', command=exportResource
    )
    fileSaveButton = Button(
        tabFile,
        text='Save Config Settings',
        command=lambda: write_save(in_var_group, in_var_names, safewordVar, False),
    )
    ...
    openLogsButton = Button(
        lSubFrame2, text='Open Logs Folder', command=lambda: explorerView(LOG_PATH)
    )
    clearLogsButton = Button(
        lSubFrame2,
        text='Delete All Logs',
        command=lambda: cleanLogs(),
        cursor='question_arrow',
    )
    logStat = Label(lSubFrame1, text=f'Total Logs: {logNum}')

    clearlogsttp = CreateToolTip(
        clearLogsButton,
        'This will delete every log (except the log currently being written).',
    )
    ...
    def cleanLogs():
        try:
            ...
            if (
                messagebox.askyesno(
                    'Confirm Delete',
                    f'Are you sure you want to delete all logs? There are currently {logNum}.',
                    icon='warning',
                )
                == True
            ):
    ...
    uniqueIDCheck = Label(
        mfSubFrame1,
        text=('Using Unique ID?: ' + ('✓' if (info_id == '0') else '✗')),
        fg=('green' if (info_id == '0') else 'red'),
    )
    uniqueIDLabel = Label(
        mfSubFrame1,
        text=('Your Unique ID is: ' + (UNIQUE_ID if (info_id == '0') else info_id)),
    )
    openMoodsButton = Button(
        mfSubFrame2,
        height=2,
        text='Open Moods Folder',
        command=lambda: explorerView(Data.MOODS),
        cursor='question_arrow',
    )
    ...
    openResourcesButton = Button(
        tabFile,
        height=2,
        text='Open Resources Folder',
        command=lambda: explorerView(Resource.ROOT),
    )
    ...
    def changeDescriptText(key: str):
        ...
        descriptLabel.configure(
            text=presetDescriptionWrap.fill(text=getDescriptText(key))
        )
    ...
    descriptLabel = Label(
        presetDescriptFrame,
        text=presetDescriptionWrap.fill(text='Default Text Here'),
        relief=GROOVE,
    )
    ...
    Label(tabAdvanced, text='Debug Config Edit', font=titleFont, relief=GROOVE).pack(
        pady=2
    )
    Label(
        tabAdvanced,
    ...
    Label(tabAdvanced, text='Troubleshooting', font=titleFont, relief=GROOVE).pack(
        pady=2
    )
    ...
    toggleLanczos = Checkbutton(
        troubleshootingFrame1,
        text='Use Lanczos instead of Antialias',
        variable=antiOrLanczosVar,
        cursor='question_arrow',
    )
    toggleInternetSetting = Checkbutton(
        troubleshootingFrame2,
        text='Disable Connection to Github',
        variable=toggleInternetVar,
        cursor='question_arrow',
    )
    toggleHibernateSkip = Checkbutton(
        troubleshootingFrame1,
        text='Toggle Tray Hibernate Skip',
        variable=toggleHibSkipVar,
        cursor='question_arrow',
    )
    toggleMoodSettings = Checkbutton(
        troubleshootingFrame2,
        text='Turn Off Mood Settings',
        variable=toggleMoodSetVar,
        cursor='question_arrow',
    )
    ...
    Label(tabAdvanced, text='Playback Options', font=titleFont, relief=GROOVE).pack(
        pady=2
    )
    ...
    offsetSlider = Scale(
        ...
        label='Pump-Scare Offset',
        orient='horizontal',
        variable=pumpScareOffsetVar,
        to=50,
        width=10,
    )
    scareOffsetButton = Button(
        ...
        command=lambda: assign(
            pumpScareOffsetVar,
            simpledialog.askinteger(
                'Offset for Pump-Scare Audio (seconds)', prompt='[0-50]: '
            ),
        ),
        cursor='question_arrow',
    )

    toggleVLC = Checkbutton(
        troubleshootingFrame4,
        text='Use VLC to play videos',
        variable=vlcModeVar,
        cursor='question_arrow',
    )
    VLCNotice = Label(
    ...
    installVLCButton = Button(
        troubleshootingFrame4,
        text="Go to VLC's website",
    ...
    Label(
        tab_hibernateType, text=HIBERNATE_TYPE_TEXT, anchor='nw', wraplength=460
    ).pack()
    ...
    if local_pp_version.split('_')[0] != webvpp.split('_')[0] and not (
        local_pp_version.endswith('DEV') or settings['toggleInternet']
    ):
        messagebox.showwarning(
            'Update Available',
                ...
                for file in files:
                    ...
                    if beyondRoot:
                        zip.write(
                            ...
                            os.path.join(Path(root).name, file),
                        )
                    else:
...
def write_save(
    ...
    if int(varList[nameList.index('timerMode')].get()) == 1:
        ...
        with open(Data.PASS_HASH, 'w') as passFile, open(
            Data.HID_TIME, 'w'
        ) as timeFile:
...
def safeCheck(
    varList: list[StringVar | IntVar | BooleanVar], nameList: list[str]
) -> bool:
    dangersList = []
    numDangers = 0
    ...
    if (
        int(varList[nameList.index('start_on_logon')].get()) == 1
        or int(varList[nameList.index('fill')].get()) == 1
    ):
    ...
    if (
        ...
        or (
            ...
            and (
                int(varList[nameList.index('hibernateMin')].get()) < 30
                or int(varList[nameList.index('hibernateMax')].get()) < 30
            )
        )
    ):
    ...
    if (
        int(varList[nameList.index('panicDisabled')].get()) == 1
        or int(varList[nameList.index('runOnSaveQuit')].get()) == 1
    ):
...
def getLiveVersion(url: str, id: int) -> str:
    ...
    if settings['toggleInternet'] == 0 or settings['toggleInternet'] == '0':
        ...
        logging.info(
            f'user has connection to github disabled. Version will not be checked. {test}'
        )
...
def toggleAssociateSettings_manual(
    ...
    for tkObject in objList:
        if (
            not tkObject.winfo_class() == 'Frame'
            and not tkObject.winfo_class() == 'Label'
        ):
...
def all_children(widget):
    return [widget] + [
        ...
        for subchild in all_children(child)
    ]
...
def themeChange(theme: str, root, style, mfont, tfont):
    if theme == 'Original' or settings['themeNoConfig'] == True:
        ...
        if theme == 'Goth':
            ...
            style.map('TNotebook.Tab', background=[('selected', '#282c34')])
            style.configure(
                'TNotebook.Tab', background='#1b1d23', foreground='MediumPurple1'
            )
        ...
        if theme == 'Bimbo':
            ...
            style.configure(
                'TNotebook.Tab', background='lightpink', foreground='deep pink'
            )
            mfont.configure(family='Constantia')
            tfont.configure(family='Constantia')
        ...
        try:
            ...
            for c in l:
                if c in filter:
                    ...
                    print(
                        f"{c}'s location in in_var_names is {loc}. Changing value of var {loc} in in_var_group..."
                    )
            ...
            messagebox.showwarning(
                'Error Loading File',
                f'There was an issue loading the pack config file. {e}',
            )
    ...
    try:
        if os.path.exists(Data.CORRUPTION_LAUNCHES):
            ...
            if confirmation:
                messagebox.showinfo(
                    'Cleaning Completed',
                    'The file that manages corruption launches has been deleted,'
